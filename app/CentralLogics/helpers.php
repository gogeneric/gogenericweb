...
//        }
    ...
    /**
     * Returns the correct image URL or a fallback image if the original image does not exist.
     *
     * @param string|null $data The image filename.
     * @param string $src The original image URL (will be overridden).
     * @param string $error_src The fallback image URL.
     * @param string $path The directory path where the image is stored.
     * @return string The correct image URL or the fallback image URL.
     */
    public static function onerror_image_helper($data, $src, $error_src, $path)
    {
        // Trim the filename to remove any leading/trailing whitespace
        $data = trim($data);

        // Check if data is set and has a valid length
        if (!empty($data) && strlen($data) > 1) {
            ...
            if (Storage::disk('public')->exists($path . $data)) {
                // Construct the correct URL by prepending 'storage/' to the path
                $correct_src = asset('storage/' . $path . $data);
                return $correct_src;
            }
        }

        // Return the error source if the file doesn't exist
        return $error_src;
    }
