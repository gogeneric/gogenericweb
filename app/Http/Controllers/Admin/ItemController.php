...
class ItemController extends Controller
{
    ...
    public function view($id)
    {
        ...
        $stockTransactions = ItemStockTransaction::where(['item_id' => $id])
            ->with(['user', 'purchaseOrder'])
            ->latest()
            ->paginate(10);
        return view('admin-views.product.view', compact('product', 'reviews', 'stockTransactions'));
    }
...
//                DB::table('items')->upsert($chunk_item, ['id', 'module_id'], ['name', 'description', 'image', 'images', 'category_id', 'category_ids', 'unit_id', 'stock', 'price', 'discount', 'discount_type', 'available_time_starts', 'available_time_ends','choice_options', 'variations', 'food_variations', 'add_ons', 'attributes', 'store_id', 'status', 'veg', 'recommended']);
    ...
    public function stock_update(Request $request)
    {
        ...
        DB::transaction(function () use ($product, $stock_count, $variations, $request) {
            // Capture original stock before update
            ...
            // Create transaction only if main stock changed
            if ($original_stock != $product->stock) {
                ItemStockTransaction::create([
                    'item_id' => $product->id,
                    'quantity' => abs($product->stock - $original_stock),
                    'type' => ($product->stock > $original_stock) ? 'credit' : 'debit',
