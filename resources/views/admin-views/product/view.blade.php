...
@section('content')
    ...
    <div class="card mt-4">
        <div class="card-header border-0">
            <h4 class="card-title">{{ translate('messages.stock_transaction_history') }}</h4>
        </div>
        <div class="table-responsive datatable-custom">
            <table class="table table-borderless table-thead-bordered table-nowrap card-table">
                <thead class="thead-light">
                    <tr>
                        ...
                        <th>{{ translate('messages.old_stock') }}</th>
                        <th>{{ translate('messages.new_stock') }}</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $transaction->type === 'credit' ? '+' : '-' }}{{ $transaction->quantity }}</td>
                        <td>{{ $transaction->old_stock }}</td>
                        <td>{{ $transaction->new_stock }}</td>
                        <td>
                            @if($transaction->purchase_order_id)
                            <a href="{{ route('admin.purchase-order.show', $transaction->purchase_order_id) }}">
                                PO #{{ $transaction->purchase_order_id }}
                            </a>
                            @elseif($transaction->user_id)
                            Manual ({{ optional($transaction->user)->f_name }} {{ optional($transaction->user)->l_name }})
                            @else
                            System
                            @endif
                        </td>
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="text-center">{{ translate('messages.no_stock_transactions_found') }}</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <!-- End Stock Transaction History Card -->
</div>
